...
> **【結論：Conclusion】**マージ先リポジトリにおいて、予め Git-LFS を有効化し、バイナリファイルのコミットを一度でも行えば（その後ハードリセットしても構わない）、Git-LFS の有効な外部リポジトリをサブツリーマージすることができる。  
In the merge destination repository, if you activate Git-LFS in advance and commit the binary file at least once (You can hard-reset it later), you can subtree-merge external repositories to which Git-LFS enabled.
